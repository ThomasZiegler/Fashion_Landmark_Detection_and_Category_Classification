...
class LandmarkBranchUpsample(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv10(x)
        lm_pos_map = x
        batch_size, num_lm, pred_h, pred_w = lm_pos_map.size()

#        lm_pos_reshaped = lm_pos_map.reshape(batch_size, num_lm, -1)
#        # y是高上的坐标，x是宽上的坐标
#        lm_pos_y, lm_pos_x = np.unravel_index(torch.argmax(lm_pos_reshaped,
#                                                           dim=2).cpu().numpy(), (pred_h, pred_w))
#        lm_pos_output = np.stack([lm_pos_x / (pred_w - 1), lm_pos_y / (pred_h - 1)], axis=2)

        # recreate np.unravel_index since np
        lm_pos_reshaped = lm_pos_map.reshape(batch_size, num_lm, -1)
        max_pos = torch.argmax(lm_pos_reshaped, dim=2)
        lm_pos_y = max_pos / pred_h
        lm_pos_x = max_pos % pred_w


        # Use Expectation instead of maximum for landmark location
#        lm_pos_reshaped = F.softmax(lm_pos_map.reshape(batch_size, num_lm, -1), dim=2)
#        x_i = torch.linspace(0,pred_w-1, steps=pred_w, device=const.device).repeat(batch_size, num_lm, pred_h)
#        y_i = torch.linspace(0,pred_h-1, steps=pred_h, device=const.device).view(-1,1).repeat(batch_size, num_lm, pred_w).view(batch_size, num_lm, -1)
#
#        lm_pos_y = torch.sum(lm_pos_reshaped*y_i, dim=2)
#        lm_pos_x = torch.sum(lm_pos_reshaped*x_i, dim=2)
#

        lm_pos_output = torch.stack([lm_pos_x.float() / (pred_w - 1),
                                     lm_pos_y.float() / (pred_h - 1)], dim=2)
