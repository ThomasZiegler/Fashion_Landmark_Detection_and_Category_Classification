...
# initialize empty (all zeros) category for LM dataset
category = pd.read_csv(base_path + 'Eval/list_eval_partition.txt', skiprows=1, sep='\s+')
category = category.drop('evaluation_status', axis=1)
...
# initialize empty (all zeros) attribute for LM dataset
attr = pd.read_csv(base_path + 'Eval/list_eval_partition.txt', skiprows=1, sep='\s+')
attr = attr.drop('evaluation_status', axis=1)
for i in range(1000):
    attr['attr_%d' % i] = 0
